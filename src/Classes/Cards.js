...
const ImageHolder = styled.div`
  ...
  filter: ${(props) =>
    props.inStock ? "brightness(100%)" : "brightness(90%) blur(4px)"};
  /* this is for the inStock=false props  */
  max-width: 25vw;
  max-height: 25vw;
  min-width: 25vw;
      ...
      >
        <Link to={{ pathname: `/products/${this.props.id}` }}>
          <ImageHolder inStock={product.inStock}>
            <img id="productImg" src={product.imgs[0]} alt={product.name}></img>
          </ImageHolder>
        </Link>
        <Absolute id="absolute">
          ...
          <OutOfStock id="outOfStock" inStock={product.inStock}>
            <h1>OUT OF STOCK</h1>
          </OutOfStock>
        </Absolute>
        <Link to={{ pathname: `/products/${this.props.id}` }}>
